...
public:
	...
	void on_update(double delta)
	{
		...
		{
			idx_target++;
			refresh_position_target();//去下一个目标点

			direction = (position_target - position).normalize();//方向向量
		}

		velocity.x = direction.x * speed * SIZE_TILE;
		velocity.y = direction.y * speed * SIZE_TILE;

		bool is_show_x_amin = abs(velocity.x) >= abs(velocity.y);//比较速度绝对值，横着走还是竖着走

		if (is_show_sketch)//剪影闪白动画
		{
			if (is_show_x_amin)
	...
	void on_render(SDL_Renderer* renderer)
	{
		...
		anim_current->on_render(renderer, point);//画动画

		if (hp < max_hp)//血条
		{//填充的矩形（下方）
			rect.x = (int)(position.x - size_hp_bar.x / 2);//居中
