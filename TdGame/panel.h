...
public:
	...
	void set_idx_tile(const SDL_Point& idx)//选中的地图格子
	{
		idx_tile_selected = idx;
	}

	void set_center_pos(const SDL_Point& pos)//面板中心位置
	{
		center_pos = pos;
	}

	void on_input(const SDL_Event& event)//处理鼠标输入。拖动，只关注下边两个量
	{
		if (!visible) return;

		switch (event.type)
		{
		case SDL_MOUSEMOTION://鼠标悬停
		{
