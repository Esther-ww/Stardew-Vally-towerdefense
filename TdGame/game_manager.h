...
private:
	...
	bool generate_tile_map_texture() {
		...
		SDL_SetTextureBlendMode(tex_tile_map, SDL_BLENDMODE_BLEND);//透明度渲染，先开混合渲染
		...
		for (int y = 0; y < map.get_height(); y++)
		{
			for (int x = 0; x < map.get_width(); x++)
			{
				...
				rect_src =
				{
					(tile.terrian % num_tile_single_line) * SIZE_TILE,//第几列
					(tile.terrian / num_tile_single_line) * SIZE_TILE,//第几行
					SIZE_TILE, SIZE_TILE
				};
