...
public:
	Tower()
	{
		...
		anim_fire_up.set_on_finished(
			[&]()
			{
				update_idle_animation();//攻击动画播完后自动切回站立动画
			});
...
private:
	void update_idle_animation()//根据面朝的方向播放站立动画
	{
		switch (facing)
		{
		case Left:
	...
	Enemy* find_target_enemy()//索敌策略：攻击范围内，走的最远的敌人   
	{
		double process = -1;
		double view_range = 0;
	...
	void on_fire()
	{
		...
		{
		case Archer:
			interval = instance->archer_template.interval[instance->level_archer];//配置文件中当前等级的攻击间隔
